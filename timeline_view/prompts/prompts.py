...
Separated from gemini_analysis.py so prompts can be
edited, versioned, and A/B tested independently.
"""
...
- If NO candidate is in the same research domain at all, set selected_predecessor_id to null.
...
{'=' * 60}
⚠️ LINEAGE CONTEXT — READ THIS CAREFULLY
{'=' * 60}
Original paper being traced: {title}
Research domain: {domain}

CRITICAL RULE: You MUST select a predecessor that is in the same research domain.
The lineage should trace how the TASK/PROBLEM was solved over time, not how a neural network architecture evolved across different tasks.
If the original paper is about machine translation, every predecessor must also be about machine translation, sequence-to-sequence modeling, or language modeling.
Do NOT select papers from other domains (discourse analysis, speech recognition, image classification, etc.) even if they use a similar architecture.
If no candidate is in the same domain, STOP and set selected_predecessor_id to null.
"""