...
def _count_all_nodes(node_list, node_type="node"):
    """Count total nodes recursively."""
    count = len(node_list)
    for node in node_list:
        if node_type == "ancestor" and node.get('ancestors'):
            count += _count_all_nodes(node['ancestors'], "ancestor")
        elif node_type == "descendant" and node.get('children'):
            count += _count_all_nodes(node['children'], "descendant")
    return count


def _build_tree_html(tree: Dict) -> str:
    """Return the D3.js tree visualization as an HTML string."""
    total_ancestors = _count_all_nodes(tree.get('ancestors', []), "ancestor")
    total_descendants = _count_all_nodes(tree.get('descendants', []), "descendant")
...
def visualize_tree_modern(tree: Dict, output_file: str = "research_tree.html") -> str:
    """
    Create modern interactive tree visualization, write to file, return file path.
    """
    html = _build_tree_html(tree)
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write(html)
    total = _count_all_nodes(tree.get('ancestors', []), "ancestor") + \
            _count_all_nodes(tree.get('descendants', []), "descendant") + 1
...
def get_tree_html(tree: Dict) -> str:
    """
    Return the D3.js tree visualization as an HTML string (no file written).
    Use with st.components.v1.html() to embed directly in Streamlit.
    """
    return _build_tree_html(tree)